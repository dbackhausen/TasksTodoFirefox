...
function doit() {
  ...
  var myMenuJson = ['xul:menuitem', {label:'Save this tab to my current task!'}];
  var mi = mainPopupSet.appendChild(utils.jsonToDOM(myMenuJson, doc, {}));
  mi.setAttribute("onclick", "console.log('Hello World!')");
  mi.setAttribute("oncommand", "console.log('Hello World!!!')");
  
  
  myMenuJson = ['xul:menuitem', {label:'Save all tabs to my current task!'}];
  ...
  mainPopupSet.appendChild(utils.jsonToDOM(myMenuJson, doc, {})); 
          ...
          menuitem.setAttribute("oncommand", 'var tab = TabContextMenu.contextTab; tab.label = tab.linkedBrowser.currentURI.spec;');
  ...
  sidebar = ui.Sidebar({
    ...
    onAttach: function (worker) {
      ...
      worker.port.on("StoreTabs", function(tab) {
        for (let tab of tabs) {
          if (!utils.isTabAlreadyStored(tab.url, taskTabs)) {
            saveTab(tab, false);
          }
        }
...
function saveTab(tab, reload) {
  if (tab != null && tab.url != undefined && tab.url !== "about:blank") {
