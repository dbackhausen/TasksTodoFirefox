  ...
  onPageLoad: function(aEvent) {
    ...
    if (taskBookmarks != null && taskBookmarks.length > 0) {
      taskBookmarks.forEach(function(bookmark) {
        if (bookmark.url == doc.location.href) {
          showPageNotification("You have bookmarked this page in your current task.");
        }
      });      
    }

    var pattern = new RegExp(/(https:\/\/www\.google\.(de|com))|(https:\/\/www\.google\.(de|com)\/\?gws_rd=ssl)|(https:\/\/www\.google\.(de|com)\/webhp\?tab=ww(.*)?)/); 
    if (pattern.test(doc.location.href)) { 
      console.log("User is visiting Google ... showing latest queries!");
      showLatestGoogleQueries();
    }
...
/*
 * This calls the script page listener and sets a notification for bookmarked pages.
 */
 ...
 */
function showLatestGoogleQueries() {
  if (taskHistory != null && taskHistory.length > 0) {
    ...
    taskHistory.forEach(function(entry) {
      var titlePattern = new RegExp(/(^(.*)Google-Suche(.*)$)|(^(.*)Google Search(.*)$)/); 
      var urlPattern = new RegExp(/((.*)\.google\.(.*)\/\?(.*)#q=(.*))/); 

    if (titlePattern.test(entry.title) 
        //if(urlPattern.test(entry.url) 
        && queries.indexOf('<a href="' + entry.url + '">') == -1) { 
        var queryString = getGoogleSearchQuery(entry.url);

        if (queryString != undefined) {
          console.log(entry.title + " (" + entry.url + ")");
          queries.unshift('<a href="' + entry.url + '">' + getGoogleSearchQuery(entry.url) + '</a>'); 
        }
      }
    });

    if (queries.length > 0) {
      worker.port.emit("ShowLatestGoogleQueries", queries);      
    }
  }
}

function getGoogleSearchQuery(url) {
  if (url.indexOf('#q=') > 0) {
