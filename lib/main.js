    ...
    onAttach: function (worker) {
      ...
      worker.port.on("TrackEvent", function(eventType, eventId) {
        if (eventType && eventId) {
          ...
          let goalId = activeGoal != null ? activeGoal._id : "";
          let taskId = activeTask != null ? activeTask._id : "";
          let url = tabs.activeTab != null ? tabs.activeTab.url : "";
      ...
      if (bookmark.url) {
        ...
        if (tArray && tArray.length > 0) {
          Array.forEach(tArray, function (tab) {
            worker = tab.attach({
              contentScriptFile: [data.url("lib/jquery/jquery-1.11.3.min.js"), data.url("firefox/page-mod.js")],
            });
          });
