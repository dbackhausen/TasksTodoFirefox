...
const WIKIPEDIA_URL_PATTERN = /((http|https):\/\/((www|de)\.)?wikipedia\.(de|org)(\/(.*))?)/;
  ...
  var saveItem = contextMenu.Item({
    ...
    onMessage: function (selectionText) {  
      if (activeTask != undefined && activeTask != null) {
        ...
        loadFileAsAttachment(activeTask.id, selectionText);
      }
    }
  });
...
function onLocationChange(url) {
  if (url && url != "about:blank" && url != "about:newtab") {
    ...
    // Load and show search engine queries when visiting one 
    // of these sites ....
    if ((new RegExp(GOOGLE_URL_PATTERN)).test(url)) {
      console.log("Get latest Google queries");
      services.loadQueries(getActiveWorker(), activeTask.id, "Google");
    } else if ((new RegExp(GOOGLE_SCHOLAR_URL_PATTERN)).test(url)) {
      console.log("Get latest Google Scholar queries");
      services.loadQueries(getActiveWorker(), activeTask.id, "Google Scholar");
    } else if ((new RegExp(BING_URL_PATTERN)).test(url)) {
      console.log("Get latest Bing queries");
      services.loadQueries(getActiveWorker(), activeTask.id, "Bing");
    } else if ((new RegExp(YAHOO_URL_PATTERN)).test(url)) {
      console.log("Get latest Yahoo queries");
      services.loadQueries(getActiveWorker(), activeTask.id, "Yahoo");
    } else if ((new RegExp(WIKIPEDIA_URL_PATTERN)).test(url)) {
      console.log("Get latest Wikipedia queries");
      services.loadQueries(getActiveWorker(), activeTask.id, "Wikipedia");
    }
  } 
}
...
function createSideBar() {  
  sidebar = ui.Sidebar({
    ...
    onAttach: function (worker) {
      ...
      worker.port.on("SetActiveTaskBookmarks", function(bookmarks) {
        ...
        if (activeTask && tabs && tabs.activeTab) {
          checkBookmarks(tabs.activeTab.url);
        }
      });
    ...
    //  var privacy = PrivateBrowsingUtils.privacyContextFromWindow(urlSourceWindow);
        ...
        notify("Attachment Download", "Downloading the task attachment " + attachment.filename + " to your desktop.")

        var uri = Services.io.newURI(attachment.url, null, null);
        persist.saveURI(uri, null, null, null, "", targetFile, /*privacy*/null);
      });
...
/*
 * This function checks the task bookmarks for the current site
 */
function checkBookmarks(url) {
 ...
 */
exports.showLatestQueries = function showLatestQueries(taskId, queries) {
  if (taskId && activeTask && taskId == activeTask.id) {
