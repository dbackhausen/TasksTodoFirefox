...
function showLatestGoogleQueries() {
  if (taskHistory != null && taskHistory.length > 0) {
    ...
    taskHistory.forEach(function(entry) { // /(https:\/\/www\.google\.(de|com)\/\?gws_rd=ssl#q=(.*))|(https:\/\/www\.google\.(de|com)\/search\?q=(.*))/
      ...
      if (titlePattern.test(entry.title) 
        ...
        && queries.indexOf('<a href="' + entry.url + '">' +  entry.title + '</a>') == -1) { 
        console.log(entry.title + " (" + entry.url + ")");
        queries.push('<a href="' + entry.url + '">' +  entry.title + '</a>'); 
      }
    });
