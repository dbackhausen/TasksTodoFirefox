...
function toggleContextMenu() {
  btnAddTab.disabled = (activeTask == null || activeTask._id == undefined || tabs.activeTab == undefined || tabs.activeTab.url == "about:blank" || utils.isTabAlreadyStored(tabs.activeTab.url, taskTabs));
  btnAddAllTabs.disabled = (activeTask == null || activeTask._id == undefined || tabs.activeTab == undefined || tabs.activeTab.url == "about:blank" || tabs.length <= 1);
};
...
//    console.log("LOCATION: " + location);
  ...
  onStateChange: function(aBrowser, aWebProgress, aRequest, aFlag, aStatus) {  
    if ((aFlag & Ci.nsIWebProgressListener.STATE_REDIRECTING) == 0) {
      if (aRequest) {
//        console.log("Recognizing redirect to: " + aRequest.name);
      }
    }
...
function onLocationChange(url) {
  if (url && url != "about:blank" && url != "about:newtab") {
    ...
    // Add a browsing history event [20150913: TOO MUCH DATA!]
  ...
  sidebar = ui.Sidebar({
    ...
    onAttach: function (worker) {
      ...
      worker.port.on("TrackEvent", function(eventType, eventId) {
        if (activeUser && activeUser != null && eventType && eventType != null && eventId && eventId != null) {
      ...
      worker.port.on("DeleteNote", function(note) {
        services.deleteNote(worker, JSON.parse(note)._id);
      });
      ...
      worker.port.on("DeleteBookmark", function(bookmark) {        
        services.deleteBookmark(worker, JSON.parse(bookmark)._id);
      });
