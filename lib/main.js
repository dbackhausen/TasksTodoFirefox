...
function onLocationChange(url) {
  if (url && url != "about:blank" && url != "about:newtab") {
    ...
    if (url.replace(lastUrl, "").length != 0) {
     ...
     // addHistoryEntry(activeTask._id, url, tabs.activeTab.title, "", tabs.activeTab.getThumbnail(), body);
  ...
  sidebar = ui.Sidebar({
    ...
    onAttach: function (worker) {
      ...
      worker.port.on("SetActiveTask", function(task) {
        ...
        if (activeTask && activeTask._id) {
          if (tabs && tabs.activeTab) {
            ...
            //   services.loadQueries(getActiveWorker(), activeTask._id, "Wikipedia");
            // }
            services.loadLogEntriesByLogQuery(getActiveWorker(), 
              {
                'taskId': activeTask._id,
                'key': "google_search",
                'limit': 5
      ...
      worker.port.on("DeleteAttachment", function(attachment) {
       // console.log("Deleting attachment " + attachment._id);
...
exports.showLatestQueries = function showLatestQueries(taskId, queries) {
  if (taskId && activeTask && taskId == activeTask._id) {
...
//   services.loadHistory(getActiveWorker(), taskId);
// }
