...
function setContextMenuItems() {
  ...
  var noteItem = contextMenu.Item({
    ...
    context: [
      ...
      contextMenu.PredicateContext(function(data) { return (activeTask != undefined && activeTask != null && activeTask._id != null && tabs.activeTab.url != "about:blank"); }),
    ],
...
function doit() {
  ...
  var myMenuJson = ['xul:menuitem', {label:'Damn right!'}];
...
//	var mainPopupSet = doc.getElementById('mainPopupSet');
  ...
  var mainPopupSet = doc.getElementById('tabContextMenu');
  
	mainPopupSet.appendChild(utils.jsonToDOM(myMenuJson, doc, {}));
  
  var myWidget = gBrowser.tabContainer.childNodes[0];
  	
//  console.log(">> " + myWidget.getAttribute('context'));
  ...
  sidebar = ui.Sidebar({
    ...
    onAttach: function (worker) {
      ...
      worker.port.on("SetActiveGoal", function(goal) {
        if (goal != null) {
          notify("Goal selected", "The goal '" + goal.title + "' has been selected!");
          ...
          notify("Goal deselected", "The goal '" + activeGoal.title + "' has been deselected!");
      ...
      worker.port.on("SetActiveTask", function(task) {
        if (task != null) {
          notify("Task selected", "The task '" + task.title + "' has been selected!");
          ...
          notify("Task deselected", "The task '" + activeTask.title + "' has been deselected!");
