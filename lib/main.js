...
const GOOGLE_TITLE_PATTERN = /(^(.*)Google-Suche(.*)$)|(^(.*)Google Search(.*)$)/;
const GOOGLE_SCHOLAR_TITLE_PATTERN = /(^(.*)Google Scholar(.*)$)/;
const BING_TITLE_PATTERN = /(^(.*)Bing(.*)$)/;
const YAHOO_TITLE_PATTERN = /(^(.*)Yahoo(.*)$)/;

const GOOGLE_URL_PATTERN = /((http|https):\/\/www\.google\.(de|com)(\/\?gws_rd=ssl|webhp?tab=ww&ei=(.*))?)/;
const GOOGLE_SCHOLAR_URL_PATTERN = /((http|https):\/\/scholar\.google\.(de|com))/;
const BING_URL_PATTERN = /((http|https):\/\/www\.bing\.(de|com)(\/\?cc=de|\/#)?)/;
const YAHOO_URL_PATTERN = /((http|https):\/\/(www|de)\.yahoo\.com)/;

const GOOGLE_SEARCH_URL_PATTERN = /((http|https):\/\/www\.google\.(de|com)(\/\?gws_rd=ssl)?(#q=(.*)))/;
const GOOGLE_SCHOLAR_SEARCH_URL_PATTERN = /((http|https):\/\/scholar\.google\.(de|com))/;
const BING_SEARCH_URL_PATTERN = /((http|https):\/\/www\.bing\.(de|com)(\/\?cc=de|\/#)?)/;
const YAHOO_SEARCH_URL_PATTERN = /((http|https):\/\/(www|de)\.yahoo\.com)/;

var seTitlePatterns = new Array ();
seTitlePatterns['Google'] = new RegExp(GOOGLE_TITLE_PATTERN);
seTitlePatterns['Google Scholar'] = new RegExp(GOOGLE_SCHOLAR_TITLE_PATTERN);
seTitlePatterns['Bing'] = new RegExp(/(^(.*)Bing(.*)$)/);
seTitlePatterns['Yahoo'] = new RegExp(/(^(.*)Yahoo(.*)$)/);

var seUrlPatterns = new Array ();
seUrlPatterns['Google'] = new RegExp(GOOGLE_URL_PATTERN); 
seUrlPatterns['Google Scholar'] = new RegExp(GOOGLE_SCHOLAR_URL_PATTERN); 
seUrlPatterns['Bing'] = new RegExp(BING_URL_PATTERN); 
seUrlPatterns['Yahoo'] = new RegExp(YAHOO_URL_PATTERN); 

var seSearchUrlPatterns = new Array ();
seSearchUrlPatterns['Google'] = new RegExp(GOOGLE_SEARCH_URL_PATTERN); 
seSearchUrlPatterns['Google Scholar'] = new RegExp(GOOGLE_SCHOLAR_SEARCH_URL_PATTERN); 
seSearchUrlPatterns['Bing'] = new RegExp(BING_SEARCH_URL_PATTERN); 
seSearchUrlPatterns['Yahoo'] = new RegExp(YAHOO_SEARCH_URL_PATTERN); 

/*
https://www.google.de/?gws_rd=ssl#q=Test+query
https://www.google.de/?gws_rd=ssl#q=Test+query&start=10
...
function setContextMenuItems() {
  ...
  var bookmarkItem = contextMenu.Item({
    ...
    onMessage: function (selectionText) {  
      if (activeTask != undefined && activeTask != null) {
        // Add bookmark
        addBookmark(activeTask.id, selectionText, selectionText, "", tabs.activeTab.getThumbnail());
      }
    }
  });
  ...
  var pageItem = contextMenu.Item({
    ...
    onMessage: function() {
      if (activeTask != undefined && activeTask != null) {
        // Add bookmark
        addBookmark(activeTask.id, tabs.activeTab.url, tabs.activeTab.title, "", tabs.activeTab.getThumbnail());
...
function onLocationChange(url) {
  if (url && url != "about:blank" && url != "about:newtab") {
    ...
    addHistoryEntry(activeTask.id, url, tabs.activeTab.title, "", tabs.activeTab.getThumbnail(), body);

    // Show message when current page is a bookmarked page
    checkBookmarks(tabs.activeTab);

    if (seUrlPatterns['Google'].test(url)) {
      console.log("Show latest Google queries");
      showLatestQueries(seTitlePatterns['Google'], seSearchUrlPatterns['Google']);
    }

    if (seUrlPatterns['Google Scholar'].test(url)) {
      console.log("Show latest Google Scholar queries");
      showLatestQueries(seTitlePatterns['Google Scholar'], seSearchUrlPatterns['Google Scholar']);
    }

    if (seUrlPatterns['Bing'].test(url)) {
      console.log("Show latest Bing queries");
      showLatestQueries(seTitlePatterns['Bing'], seSearchUrlPatterns['Bing']);
    }

    if (seUrlPatterns['Yahoo'].test(url)) {
      console.log("Show latest Yahoo queries");
      showLatestQueries(seTitlePatterns['Yahoo'], seSearchUrlPatterns['Yahoo']);
    }
  } 
}
...
function createSideBar() {
  sidebar = ui.Sidebar({
    ...
    onAttach: function (worker) {
       ...
       */
      worker.port.on("LoadCompletedGoals", function(user) {
        services.loadCompletedGoals(worker, user.id);
      });
       ...
       */
      worker.port.on("LoadCompletedTasks", function(goal) {
        services.loadCompletedTasks(worker, goal.id);
      });
      ...
      worker.port.on("LoadHistory", function(task) {
        console.log("Loading task history for task " + task.id);
      ...
      worker.port.on("LoadAttachments", function(task) {
        services.loadAttachments(worker, task.id);
      });
      ...
      worker.port.on("AddAttachment", function(task, data, filename, filetype) {
        services.addAttachment(worker, task.id, data, filename, filetype);  
      });
      ...
      worker.port.on("DeleteAttachment", function(attachment) {
       // console.log("Deleting attachment " + attachment.id);
        services.deleteAttachment(worker, attachment.id);
      });
    },
    onShow: function () {
...
function checkBookmarks(tab) {
  hidePageNotification();

  if (taskBookmarks != null && taskBookmarks.length > 0) {
...
function showPageNotification(message) {
 // console.log("Show page notification");
...
function hidePageNotification() {
 // console.log("Hide page notification");
  worker = tabs.activeTab.attach({
    contentScriptFile: [data.url("js/jquery-1.10.2.min.js"), data.url("firefox/page-mod.js")]
  });
...
/**
 * SHow latest google searches.
 */
function showLatestQueries(titlePattern, urlPattern) {
  if (taskHistory != null && taskHistory.length > 0) {
    worker = tabs.activeTab.attach({
      contentScriptFile: [data.url("js/jquery-1.10.2.min.js"), data.url("firefox/page-mod.js")]
    });
...
//console.log(titlePattern.test(entry.title));  
//console.log(urlPattern.test(entry.url));

      if (/*titlePattern.test(entry.title) && */urlPattern.test(entry.url)) {
        // get the query string from url
        var queryString = getQueryString(entry.url);

        if (queryString && queries.indexOf(queryString) == -1) { // dismiss duplicates
          // add query string to array
          queries.unshift(queryString);
    ...
    if (queries.length > 0) {
      worker.port.emit("ShowLatestQueries", queries);      
    }
  }
}
...
function getQueryString(url) {
  if (url.indexOf('#q=') > 0) {
    return url.slice(url.indexOf('#q=') + 3, url.length);
  } else {
    var vars = [], hash;
 ...
 * Add history event
 */
function addHistoryEntry(taskId, url, title, description, thumbnail, body) {
  var json = { 
    "taskId" : taskId,
    "url" : url,
    "title" : title, 
