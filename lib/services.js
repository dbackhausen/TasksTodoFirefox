 ...
 */
function loadGoalNotes(worker, goalId) {
  if (goalId) {
    var notes = datastore.readGoalNotes(goalId);
    worker.port.emit("GoalNotesLoaded", notes);
  }
} exports.loadGoalNotes = loadGoalNotes;

/**
 * Loads a specific note.
 */
function loadGoalNote(worker, noteId) {
  if (noteId != null) {
    var note = datastore.readNote(noteId);
    worker.port.emit("GoalNoteLoaded", note);
  }
} exports.loadGoalNote = loadGoalNote;

/**
 * Adds a new note.
 */
function addGoalNote(worker, note) {
  if (note != null) {
    note = datastore.createNote(note);
    worker.port.emit("GoalNoteAdded", note);
  }
} exports.addGoalNote = addGoalNote;
...
} exports.updateGoalNote = updateGoalNote;
...
} exports.deleteGoalNote = deleteGoalNote;
 ...
 */
function loadGoalSummary(worker, goalId) {
  if (goalId != null) {
    var summary = datastore.readLogEntriesByGoal(goalId, 30)
    worker.port.emit("GoalSummaryLoaded", summary);
  }
} exports.loadGoalSummary = loadGoalSummary;
