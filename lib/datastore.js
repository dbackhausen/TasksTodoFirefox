 ...
 */
exports.readLogEntries = function readLogEntries(taskId, action) {
  var stmt = dbConn.createStatement("SELECT e.id, e.created FROM logentries e, logparams p WHERE e.id = p.logentry AND e.action = :action AND p.key = 'taskId' AND p.value = :task");
  var results = [];
  ...
  try {
    ...
    while (stmt.step()) {
      ...
      stmt = dbConn.createStatement("SELECT * FROM logparams WHERE logentry = :logEntryId");
...
/**
 * Load a specific log entry.
 */
exports.readLogEntry = function readLogEntry(logEntryId) {
  var stmt = dbConn.createStatement("SELECT * FROM logentries WHERE id = :logEntryId");
  var logEntry = null;
