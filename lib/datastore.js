...
function initialize() {
  ...
  //var dbFile = FileUtils.getFile("Home", ["taskstodo.sqlite"]);
  var dbFile = FileUtils.getFile("ProfD", ["taskstodo.sqlite"]);
  ...
  if (!dbExists) {
    ...
    try {
      ...
      dbConn.executeSimpleSQL("CREATE TABLE notes (id INTEGER PRIMARY KEY AUTOINCREMENT, goal INTEGER, task INTEGER, body TEXT, created TEXT, modified TEXT)");
      dbConn.executeSimpleSQL("CREATE INDEX notesgoalindex ON notes(goal)");
 ...
 * Load all notes.
 */
exports.readGoalNotes = function readGoalNotes(goalId) {
  var stmt = dbConn.createStatement("SELECT * FROM goalnotes WHERE goal = :goalId ORDER BY created DESC");
  var results = [];
 ...
 */
exports.readGoalNote = function readGoalNote(noteId) {
  var stmt = dbConn.createStatement("SELECT * FROM goalnotes WHERE id = :noteId");
  var note = null;
 ...
 * Store a new note.
 */
exports.createGoalNote = function createGoalNote(note) {
  let noteId = -1;
  var stmt = dbConn.createStatement("INSERT INTO goalnotes (goal, body, created) VALUES(:goal, :body, :created)");
  
  try {
    let now = new Date();

    stmt.params.goal = note.goal;
    stmt.params.body = note.body;
    stmt.params.created = now.toISOString();

    stmt.execute();
    noteId = getLatestId("goalnotes");
    note._id = noteId;
  } catch (ex) {
    console.error(ex);
  } finally {
 ...
 */
exports.updateGoalNote = function updateGoalNote(note) {
  var stmt = dbConn.createStatement("UPDATE goalnotes SET goal = :goal, body = :body, modified = :modified WHERE id = :id");
 ...
 */
exports.deleteGoalNote = function deleteGoalNote(noteId) {
  var stmt = dbConn.createStatement("DELETE FROM goalnotes WHERE id = :id");
 ...
 */
exports.readGoalNotes = function readGoalNotes(goalId) {
  var stmt = dbConn.createStatement("SELECT * FROM notes WHERE goal = :goalId ORDER BY created DESC");
  var results = [];
 ...
 * Load all notes.
 */
exports.readTaskNotes = function readTaskNotes(taskId) {
...
exports.createNote = function createNote(note) {
  let noteId = -1;
  var stmt = dbConn.createStatement("INSERT INTO notes (goal, task, body, created) VALUES(:goal, :task, :body, :created)");
...
exports.updateNote = function updateNote(note) {
  var stmt = dbConn.createStatement("UPDATE notes SET goal = :goal, task = :task, body = :body, modified = :modified WHERE id = :id");
...
/**
 * Load all log entries regarding to a certain goal.
 */
exports.readLogEntriesByGoal = function readLogEntriesByGoal(goalId, limit) {
  limit = (limit && (limit >= 0 && limit <= 100)) ? limit : 30;
  var stmt = dbConn.createStatement("SELECT e.id, e.action, e.created FROM logentries e, logparams p WHERE e.id = p.logentry AND p.key = 'goalId' AND p.value = :goal ORDER BY e.created DESC LIMIT 0," + limit);
  var substmt = dbConn.createStatement("SELECT * FROM logparams WHERE logentry = :logEntryId");

  var results = [];
    
  try {
    stmt.params.goal = goalId;
    
    while (stmt.step()) {
      let logEntryId = stmt.row.id;
      let action = stmt.row.action;
      let created = stmt.row.created;
      ...
      while (substmt.step()) {        
        ...
        if (substmt.row.key === "goalId" && substmt.row.value && substmt.row.value.length > 0) {
          let goal = exports.readGoal(substmt.row.value);
          if (goal != null) {
            params.push({
              key: "goalTitle",
              value: goal.title
        ...
        if (substmt.row.key === "taskId" && substmt.row.value && substmt.row.value.length > 0) {
          let task = exports.readTask(substmt.row.value);
          if (task != null) {
            params.push({
              key: "taskTitle",
              value: task.title
